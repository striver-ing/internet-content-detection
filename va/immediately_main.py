...
def main():
    ...
    #  更新符合日期条件的任务状态 未做
    sql = 'update tab_ivms_task_info t set t.task_status = 501 where sysdate >= t.monitor_start_time and sysdate <= t.monitor_end_time and task_type = 801'
    db.update(sql)

    # 更新关键词状态 未做
    sql = 'update tab_ivms_task_keyword k set k.finish_status = 601 where k.task_id in (select t.task_id from tab_ivms_task_info t where sysdate >= t.monitor_start_time and sysdate <= t.monitor_end_time and task_type = 801)'
    db.update(sql)
    ...
    while True:
        ...
        sql = 'select t.task_id from TAB_IVMS_TASK_INFO t where sysdate >= t.monitor_start_time and sysdate <= t.monitor_end_time and t.task_status = 501 and task_type = 801' # 801 紧急任务
        ...
        while True:
            ...
            def end_callback():
                ...
                if not results:
                    ...
                    # 更新任务状态 做完
                    sql = 'update TAB_IVMS_TASK_INFO set task_status = 503, task_type = 802 where task_id = %d'%task_id
                    db.update(sql)
                    log.info('\n********** VA end **********')
