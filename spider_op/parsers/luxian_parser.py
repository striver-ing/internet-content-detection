...
def parser(url_info):
    ...
    for url in urls:
        if re.match("http", url):
            ...
            new_url = 'http://www.luxian.gov.cn'+url
    ...
    log.debug('''
                ...
                watched_count       = %s
                content             = %s
             ''' % (depth+1, source_url, title, release_time, author, watched_count, content))

    if content and title:
        base_parser.add_op_info('op_content_info', website_id, url=source_url, title=title, release_time=release_time,
                                author=author, watched_count=watched_count, content=content)
