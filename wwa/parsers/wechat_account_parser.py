...
def add_root_url(parser_params = {}):
    ...
    if keyword:
        url = 'http://weixin.sogou.com/weixin?type=1&query=%s&ie=utf8&s_from=input&_sug_=y&_sug_type_='%keyword
...
def parser(url_info):
    ...
    for account_block in account_blocks:
        ...
        iamge_url = tools.get_info(account_block, regex, fetch_one = True)
        ...
        is_verified = 1 if tools.get_info(account_block, regex, fetch_one = True) else 0
        ...
        log.debug('''
            ...
            是否加V（是否认证） %s
            二维码              %s
            '''%(name, account_id, account_url, iamge_url, article_count, summary, certification, is_verified, barcode_url))
...
if __name__ == '__main__':
    url_info = {
            ...
            "status" : 2,
            "site_id" : 3,
            "url" : "http://weixin.sogou.com/weixin?type=1&query=小罗恶搞&ie=utf8&s_from=input&_sug_=y&_sug_type_=",
            "depth" : 0,
            "remark" : ''
        }
