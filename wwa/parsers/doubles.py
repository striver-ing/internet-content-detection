...
if __name__ == '__main__':
    # root_url = 'http://c.tieba.baidu.com/p/2583711475?pid=38682939717&see_lz=1'
    # html = tools.get_html_by_urllib(root_url)
    # infos = '</ul></div><div class="d_post_content_main ">(.*?)</cc>'
    # infos = tools.get_info(html, infos) src="http://img.jkyd.net/uploads/cj_img/xiuxian/yulebagua/20160815/es2znksqee3.jpg"
    # for html in infos:
    #     title = '<cc>.*?<div id=".*?" class=".*?">(.*?)名'
    #     title = tools.get_info(html, title)
    #     title = ''.join(title)+'名'
    #
    #     url = '<cc>.*?<div id=".*?" class=".*?">.*?名(.*?)<br>'
    #     url = tools.get_info(html, url)
    #     url = ''.join(url)
    #     url = tools.del_html_tag(url)
    #
    #     if title and url:
    #         print(title,'****',url)
    url = 'http://www.jkyd.net/xiuxian/171399.html'
    html = tools.get_html_by_urllib(url)
    title = '<td>.*?</td>'
    title = tools.get_info(html, title)
    a = '></p><p>(.*?)</p>'
    a = tools.get_info(title,a)
    for i in a:
        print(i)
    #     title = tools.get_info(html, title)
    #     title = ''.join(title)+'名'
